...
def mle_infer(args, dispersion):
    ...
    if pvalue_value <= MIN_PVALUE_FLOAT32:
        logger.warning(
            'P-value (%g) below float32 minimum for gene=%s sample=%s impact=%s; replacing with %g',
            pvalue_value,
            gene_term,
            sample_term,
            impact_term,
        ...
        pvalue_value = MIN_PVALUE_FLOAT32
